...
# Itérer sur les données
    ...
    # Initialiser le compteur pour cette région si ce n'est pas déjà fait
        if nom_region not in nb_zones_litigieuses_non_vides_par_region:
             nb_zones_litigieuses_non_vides_par_region[nom_region] = 0

    # Compter le nombre de zones litigieuses non vides pour cet élément
        for zone in item['limite_litige_village']:
            print(zone["zone_litigee"])
           #if zone['zone_litigee'] != '':
             # nb_zones_litigieuses_non_vides_par_region[nom_region] += 1

# Afficher le nombre de zones litigieuses non vides par région
