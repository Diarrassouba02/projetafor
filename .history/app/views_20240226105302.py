...
#   Etude anciens sites région
    ...
    nombre_zones_litigieuses_par_region = {}

# Parcourir chaque élément de reglitige
    for item in reglitige:
    # Extraire le nom de la région et la liste des villages litigieux pour cet élément
        nom_region = item['nom_region']
        villages_litigieux = item['limite_litige_village']
    
    # Initialiser le compteur de zones litigieuses pour cette région
        nombre_zones_litigieuses = 0
    
    # Parcourir chaque village litigieux pour compter les zones litigieuses non vides
        for village in villages_litigieux:
            zone_litigiee = village['zone_litigee']
            if zone_litigiee != '':
               nombre_zones_litigieuses += 1
    
    # Ajouter le nombre de zones litigieuses au dictionnaire par région
        if nom_region in nombre_zones_litigieuses_par_region:
           nombre_zones_litigieuses_par_region[nom_region] += nombre_zones_litigieuses
        else:
            nombre_zones_litigieuses_par_region[nom_region] = nombre_zones_litigieuses

# Afficher le dictionnaire résultant
    print(nombre_zones_litigieuses_par_region)
