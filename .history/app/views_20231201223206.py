...
from reportlab.lib.units import inch,image_reader
...
def rapport_pdf(request,pk):
    ...
    image_url = static('/images/repuplique.jpg')
    image_reader = ImageReader(image_url)
    #image_path = staticfiles_storage.path('images/repuplique.jpg')
    c.drawInlineImage(image_url, A4[0]/2 - inch, A4[1] - inch, width=1.5*inch, height=1.5*inch)
    textob=c.beginText()
    textob.setTextOrigin(inch, A4[1] - inch - 1.5*inch)  # Déplacer le texte en dessous de l'image
