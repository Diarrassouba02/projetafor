...
def rapport_pdf(request, pk):
   ...
   "est en annexe les déclarations ci-après:"]
    ...
    text6 = c.beginText()
    text6.setTextOrigin(1.2*cm ,600)
    for line1 in text_lines6:
        parts = line1.split(":")  # Sépare la partie "Nom et prénoms du déclarant" du reste du texte
        text6.textLine(parts[0])  # Dessine la première partie sans gras
        c.setFont("Helvetica-Bold", 12)  # Utilise une police en gras pour la deuxième partie
        text6.textLine(parts[1])  # Dessine la deuxième partie en gras
        #c.setFont("Helvetica", 12)  # Reviens à la police normale pour le reste du texte
    c.drawText(text6)
