...
def modifie(request, histoire_id):
    # Récupérer l'instance existante à modifier
    histoire = get_object_or_404(Histoire, pk=histoire_id)

    # Le reste de votre code reste essentiellement le même avec quelques ajustements
    ...
    if request.method == 'POST':
        ...
        # Mettez à jour les champs de l'instance existante avec les nouvelles données
        histoire.nom_region = request.POST.get('region')
        histoire.nom_departement = request.POST.get('department')
        # ... (Mettez à jour les autres champs)

        histoire.save()

        messages.success(request, "Village modifié avec succès")
        return redirect('nom_de_la_vue_de_liste_des_villages')  # Remplacez par le nom de votre vue de liste
    else:
        # Pré-remplir le formulaire avec les données existantes
        context = {
            'histoire': histoire,
            # ... (Autres données à envoyer au template)
        }
        return render(request, 'nom_de_votre_template_de_modification.html', context)
