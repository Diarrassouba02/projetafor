...
def rapport_pdf(request, pk):
    ...
    # Chemin de l'image
    image_path = "/home/afor/Bureau/BASE/static/images/repuplique.jpg"

    # Ouvrir l'image avec Pillow (PIL)
    image = Image.open(image_path)
    if hasattr(image, '_getexif'):  # Vérifier si les informations Exif sont disponibles
        exif = image._getexif()
    ...
    # Sauvegarder temporairement l'image dans le flux de bytes
    image_buf = io.BytesIO()
    image.save(image_buf, format='JPEG')

    # Utiliser le chemin du flux de bytes dans ReportLab
    c.drawImage(ImageReader(image_buf), A4[1]/2 - inch, A4[1] - inch, width=1.5*inch, height=1.5*inch)

    # Continuer avec le reste du code
    c.showPage()
    c.save()

    # Rembobiner le flux de bytes
    buf.seek(0)

    # Retourner la réponse HTTP avec le fichier PDF en pièce jointe
