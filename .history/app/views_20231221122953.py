...
def visualisationt(request):
    pertrouve_data = Histoire.objects.values('personne_trouve').annotate(
        nbpertrouve=Count('personne_trouve', distinct=True))
    labels = [item['personne_trouve'] for item in pertrouve_data]
    data = [item['nbpertrouve'] for item in pertrouve_data]
    return render(request, 'visualisation.html',{'labels': labels, 'data': data})