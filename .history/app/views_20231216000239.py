...
def visualisationt(request):
    ...
    for nomregion in nomregions:
        ...
        nbsousprefectures = len(set(Histoire.objects.filter(nom_region=region_name).values_list('sous_prefecture', flat=True)))
        nbvillages = len(set(Histoire.objects.filter(nom_region=region_name).values_list('nom_village', flat=True)))
        print(nbdepartements)


        data.append({
            'region': region_name,
            'nbdepartements': nbdepartements,
            'nbsous_prefectures': nbsousprefectures,
    ...
    nbsous_prefectures = [entry['nbsous_prefectures'] for entry in data]
    nbvillages=[entry['nbvillages'] for entry in data]
    #print(nbdepartements)
    #departements = [entry['departements'] for entry in data]
    #sous_prefectures = [entry['sous_prefectures'] for entry in data]
    #villages = [entry['villages'] for entry in data]

    context={"labels":labels,'region': region_name,'nbdepartements': nbdepartements,'nbsous_prefectures': nbsousprefectures,'nbvillages': nbvillages}
