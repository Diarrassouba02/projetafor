...
def visualisationt(request):
    ...
    if  request.method == 'POST':
        ...
        for sp in histoires_region.values('sous_prefecture','nom_departement').distinct():
            ...
            departement_data = {
                ...
                'nbv': len(set(histoires_region.filter(sous_prefecture= sous_prefecture_name).values_list('nom_village', flat=True)))
            }
    ...
    dep=[item['dep'] for item in data1]
    nom_sp=[item['nom_sp'] for item in data1]
    nbv=[item['nbv'] for item in data1]

    datasets = {}
    for entry in data1:
        dep = entry['dep']
        if dep not in datasets:
            datasets[dep] = {'label': dep, 'data': []}
        datasets[dep]['data'].append({'x': entry['nom_sp'], 'y': entry['nbv']})

    context={'labels':labels,'nbsp':nbsp,'nbv':nbv,'dep':dep,
    'nom_sp':nom_sp,'nbv':nbv,'datasets': datasets}
