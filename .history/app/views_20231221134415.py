...
def index(request):
    pertrouve=Histoire.objects.values('personne_trouve')
    ...
    'nbvillages': nbvillages,'ndp':ndp,'nsp':nsp,'nrg':nrg,'pertrouve': pertrouve,
...
def visualisationt(request):
    pertrouve_data = Histoire.objects.values(' mode_accès_terre').annotate(
        nb=Count('mode_accès_terre', distinct=True))
    print(pertrouve_data)
    labels1 = [item['mode_mise_a_diposition'] for item in pertrouve_data]
    data1 = [item['nb'] for item in pertrouve_data]
    return render(request, 'visualisation.html',{'labels1': labels1, 'data1': data1})