...
def rapport_pdf(request,pk):
    ...
    can=canvas.Canvas(buf,pagesize=A4,bottomup=0)

    can.drawInlineImage(image,(A4[0] - 120) / 2 , A4[1] - 120 - 1*cm, width=120,height=120, preserveAspectRatio=True, mask='auto')
    can.showPage()
    can.save()
    buf.seek(0)
    return FileResponse(buf, as_attachment=True, filename='hello_world.pdf')
