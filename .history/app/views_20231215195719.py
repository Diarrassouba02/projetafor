...
def visualisationt(request):
    ...
    for nomregion in  nomregions:
        ...
        nbdepartements = Histoire.objects.filter(nom_region=region_name).values('nom_departement').distinct().count()
        nbsous_prefectures = Histoire.objects.filter(nom_region=region_name).values('sous_prefecture').distinct().count()
        nbvillages = Histoire.objects.filter(nom_region=region_name).values('nom_village').distinct().count()

        data.append({
            'region': region_name,
            'departements': nbdepartements,
            'sous_prefectures': nbsous_prefectures,
            'villages': nbvillages,
            })
        print(data)
