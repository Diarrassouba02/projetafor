...
def rapport_pdf(request, pk):
   ...
   "est en annexe les déclarations ci-après:"]
    ...
    c.rect(1.1 * cm, 1.1 * cm, 80, 18, fill=True)
    c.setFillColor(colors.black)
    c.drawString(4.3*cm,1.5*cm,"Quels sont les npms des chefs qui se sont succédés a la tête du village? pouvez-vous")
    c.drawString(4.3*cm,2*cm,"nous donner les dates d'accession au pouvoir de ces chefs?")
    #c.drawString(4.3*cm,2.5*cm,"Qu'est ce qui les liait au fondateur?")
