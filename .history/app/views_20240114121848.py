...
def rapport_pdf(request, pk):
   ...
   "est en annexe les déclarations ci-après:"]
    ...
    for i in histoire.succeseur_nom_prenon_date:
        if i["nom"]:
           if i["prénom"]:
              ...
              #else:
                 # suc.append({'nom': i["nom"],'prénom':i["prénom"],'accession_date':'Inconnue',})
           #else:
               #suc.append({'nom': i["nom"],'prénom':'Inconnu','accession_date':i["accession_date"],})
    ...
    c.drawString(1.2*cm,2.7*cm,"La liste des successeurs dans l'ordre nom, prénom, d'aide accession")
