...
def index(request):
    ...
    quad = (
        Histoire.objects.annotate(
            qualite_combined=Case(
                When(qualite_declarant='Chef De Village', then=Value('Chef du village')),
                When(qualite_declarant='Chef Du Village', then=Value('Chef du village')),
                default='qualite_declarant',
                output_field=IntegerField(),
            )
        )
        .values('qualite_combined')
        .annotate(nb=Count('qualite_combined'))
        .order_by('-nb')[:5]
    )
    print(quad)
