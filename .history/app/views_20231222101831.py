...
def visualisationt(request):
    if  request.method == 'POST':
        ...
        detail = Histoire.objects.values(nom_regions_select,'nom_departement').annotate(
        nb=Count('sous_prefecture', distinct=True))
        print(detail)

    return render(request, 'visualisation.html',{'detail':detail})