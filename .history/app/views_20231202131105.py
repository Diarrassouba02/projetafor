...
def rapport_pdf(request,pk):
    ...
    can=canvas.Canvas(image_path,pagesize=A4,bottomup=0)

    can.drawImage(image_path,(A4[0] - 120) / 2 , A4[1] - 120 - 1*cm, width=120,height=120, preserveAspectRatio=True, mask='auto')
    can.showPage()
    can.save()
    #buf.seek(0)
    return FileResponse(open("hello_world.pdf", "rb"), as_attachment=True, filename='hello_world.pdf')
