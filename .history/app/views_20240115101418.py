...
def rapport_pdf(request, pk):
   ...
   "est en annexe les déclarations ci-après:"]
    ...
    c.rect(1.1 * cm, 1.1 * cm, 80, 18, fill=True)
    c.setFillColor(colors.black)
    c.drawString(4.3*cm,1.5*cm,"Depuis quand Ia localité est erigee en village officiellement? Par quelle autorité a-t-elle")
    c.drawString(4.3*cm,2*cm,"été erigee (Administration du territoire, ARSO, AVB etc.) Pouvez-vous produire l'acte de création ")
    c.drawString(4.3*cm,2.5*cm,"Qu'est ce qui les liait au fondateur?")
