...
def visualisationt(request):
    if  request.method == 'POST':
        ...
        print(nom_regions_select)
        histoires_region= Histoire.objects.values(nom_region=nom_regions_select)
        region_aggregated = histoires_region.values('nom_region').annotate(
        nombre_departements=Count('nom_departement', distinct=True),nombre_villages=Count('nom_village', distinct=True))
