...
from django.shortcuts import render, redirect
from .models import Successeur  # Assurez-vous d'importer votre modèle Successeur
...
def formulairetranscription(request):
    if request.method == 'POST':
        ...
        for i in range(successor_count):
            ...
            # Vous pouvez maintenant effectuer des opérations sur ces données, par exemple, les enregistrer dans la base de données
            # Exemple : successeur = Successeur(name=name, first_name=first_name, accession_date=accession_date)
            # successeur.save()
