...
def rapport_pdf(request,pk):
    ...
    c.drawImage(image_path,1*cm, 1*cm, width=2*cm, height=2*cm,preserveAspectRatio=True, mask='auto')
    c.showPage()
    c.save()
    buf.seek(0)
    return FileResponse(c,as_attachment=True,filename='proces_verbale.pdf')
