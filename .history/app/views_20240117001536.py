   ...
   "est en annexe les déclarations ci-après:"]
    ...
    for i in histoire.campement_nom_origine:
        if i['peuple']:
           if i['nom']:
               if i['origine']:
                   text_lines14=f"Peuple: {i['peuple']}, nom du campement: {i['nom']}, origine de ce peuple: { i['origine']}. "+text_lines14
               else:
                   text_lines14=f"Peuple: {i['peuple']}, nom du campement: {i['nom']}, origine de ce peuple: Origine Inconnu. "+text_lines14
           else:
                if i['origine']:
                   text_lines14=f"Peuple: {i['peuple']}, nom du campement: inconnu, origine de ce peuple: { i['origine']}. "+text_lines14
                else:
                   text_lines14=f"Peuple: {i['peuple']}, nom du campement: inconnu, origine de ce peuple: Origine Inconnu. "+text_lines14
        else:
            if i['nom']:
               if i['origine']:
                   text_lines14=f"Peuple: Inconnu, nom du campement: {i['nom']}, origine de ce peuple: { i['origine']}. "+text_lines14
               else:
                   text_lines14=f"Peuple: Inconnu, nom du campement: {i['nom']}, origine de ce peuple: Origine Inconnu. "+text_lines14
            else:
                if i['origine']:
                   text_lines14=f"Peuple: Inconnu, nom du campement: inconnu, origine de ce peuple: { i['origine']}. "+text_lines14
                else:
    ...
    c.drawString(1.2*cm,16*cm,f"Nombre de campement:{ len(histoire.campement_nom_origine)}")
    paragraphe1= Paragraph(text_lines14, style)
    paragraphe1.wrapOn(c, 530,40)
    paragraphe1.drawOn(c, 1.2 * cm, 16.5*cm)
    c.drawString(1*cm,840,"EODTV 2")
    ...
    paragraphe1.drawOn(c, 1.2 * cm, 15.5*cm)
    c.drawString(1*cm,840,"EODTV 2")
    c.drawString(19*cm,840,"page8")
    c.showPage()
    #neuvième page
    c.setStrokeColor(colors.green)
    c.rect(1*cm, 1*cm,535, 28*cm)
    c.setFillColor(colors.green)
    c.rect(1 * cm, 1 * cm, 540, 0.80*cm, fill=True)
    c.setFillColorRGB(1, 1, 1)  # Blanc en RGB
    c.rect(1.1 * cm, 1.1 * cm, 82, 18, fill=True)
    ...
    c.drawString(10.5*cm,750,"Signature du Commissafte-Enqueteur")
    c.drawString(1.5*cm,850,"Comité Villageois de Gestion Foncière Rurale de")
    c.drawString(1*cm,840,"EODTV 2")
    c.drawString(19*cm,840,"page9")
