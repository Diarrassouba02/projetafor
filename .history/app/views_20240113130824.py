...
def rapport_pdf(request, pk):
   ...
   "est en annexe les déclarations ci-après:"]
    ...
    c.rect(1.1 * cm, 1.1 * cm, 80, 18, fill=True)
    c.setFillColor(colors.black)
    c.drawString(4.5*cm,1.5*cm,"D'où venait-il ? A-t-il trouvé des personnes sur les lieux ? Si oui,lesquelles (villages,")
    c.drawString(4.5*cm,2*cm,"groupement de personnes)? ")
    c.drawString(4.5*cm,2.5*cm,"A-t-il passé des accords avec ces derniers?")
