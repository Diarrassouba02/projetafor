...
def rapport_pdf(request,pk):
    #buf=io.BytesIO()
    c=canvas.Canvas(pagesize=A4,bottomup=0)
    image_path = staticfiles_storage.path('images/repuplique.jpg')
    #image = Image.open(image_path)
    #image = image.rotate(, expand=True)
    c.drawImage(image_path, 1*cm, 1*cm,mask='auto')

    c.showPage()
    c.save()
    #buf.seek(0)
    return FileResponse(as_attachment=True,filename='proces_verbale.pdf')