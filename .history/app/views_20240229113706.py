...
def index(request):
    ...
    epoqued= Histoire.objects.values('epoque_installation').annotate(
        nb=Count('epoque_installation', distinct=True)).order_by('-nb')[:10]
    ...
    activite_fon= Histoire.objects.values('activite_fondateur').annotate(
        nb=Count('activite_fondateur', distinct=True)).order_by('-nb')[:10]


    dateoff= Histoire.objects.values('dateoff').annotate(
        nb=Count('dateoff', distinct=True)).order_by('-nb')[:10]
    ...
    autoriteoff= Histoire.objects.values('autoriteoff').annotate(
        nb=Count('autoriteoff', distinct=True)).order_by('-nb')[:10]
...
#   Etude anciens sites région
    ...
    'dicregsuc':dicregsuc,'dicregli':dicregli,'resultat_final':resultat_final}
