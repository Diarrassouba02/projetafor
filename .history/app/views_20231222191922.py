...
def visualisationt(request):
    if  request.method == 'POST':
        ...
        region_aggregated1=region_aggregated.values('nom_departement').annotate(
            nombre_departements=len(set(Histoire.objects.filter('nom_departement').values_list('nom_departement', flat=True))))
        print(region_aggregated1)
