...
def visualisationt(request):
    ...
    if  request.method == 'POST':
        ...
        data=[]
        histoires_region= Histoire.objects.filter(nom_region=nom_regions_select).distinct()
        print(histoires_region)
        #nbdepartements =len(set(Histoire.objects.filter(nom_region=region_name).values_list('nom_departement', flat=True)))
        region_aggregated = histoires_region.values('nom_region','nom_departement','sous_prefecture').distinct()
        depatement_name = region_aggregated['nom_departement']
        for departement in depatement_name:
            departement_data = {
                'nom_region': nom_regions_select,
                'nom_departement':depatement_name,
                'nombre_sous_prefecture': len(set(region_aggregated.filter(nom_departement= depatement_name).values_list('sous_prefecture', flat=True)))
