...
def visualisationt(request):
    ...
    for nomregion in nomregions:
        ...
        departements = Histoire.objects.filter(nom_region=region_name).values('nom_departement').distinct().count()
        sous_prefectures = Histoire.objects.filter(nom_region=region_name).values('sous_prefecture').distinct().count()
        villages = Histoire.objects.filter(nom_region=region_name).values('nom_village').distinct().count()
