...
def index(request):
    ...
    mode_acces = Histoire.objects.values('mode_accès_terre').annotate(
        nb=Count('mode_accès_terre', distinct=True))
    ...
    mise_a_disp = Histoire.objects.values('mode_mise_a_diposition').annotate(
        nb=Count('mode_mise_a_diposition', distinct=True))
...
def visualisationt(request):
    if request.method == 'POST':
        nom_region_select = request.POST.get('region')
        data = []

        # Filtrer les objets correspondant à la région sélectionnée
        histoires_region = Histoire.objects.filter(nom_region=nom_region_select)

        # Obtenir le nombre de départements distincts pour la région
        nb_departements = histoires_region.values('nom_departement').distinct().count()

        # Obtenir le nombre de sous-préfectures distinctes par département
        region_aggregated = histoires_region.values(
            'nom_region', 'nom_departement', 'sous_prefecture'
        ).distinct()

        # Obtenir la liste des départements
        departements = histoires_region.values_list('nom_departement', flat=True).distinct()

        # Créer un dictionnaire pour chaque département
        for departement in departements:
            departement_data = {
                'nom_region': nom_region_select,
                'nom_departement': departement,
                'nombre_sous_prefecture': region_aggregated.filter(nom_departement=departement).count(),
                'nombre_villages': histoires_region.filter(nom_departement=departement).count()
            }
