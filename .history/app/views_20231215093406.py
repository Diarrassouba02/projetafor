...
def index(request):
    ...
    regions = Histoire.objects.values('nom_region').aaggregate(
    departements=Count('sous_prefecture',distinct=True))
    print(regions)

    # Obtenez les données agrégées à partir du modèle Histoire
    regions_data = Histoire.objects.values('nom_region').aggregate(
...
def visualisationt(request):
    ...
    departements=Count('nom_departement', distinct=True),
    sous_prefectures=Count('sous_prefecture'),
    villages=Count('nom_village', distinct=True)
    )
  # Préparez les données pour le rendu sur le template
    labels = [entry['nom_region'] for entry in regions_data]
    departements = [entry['departements'] for entry in regions_data]
