...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function cloneAndAppendSuccessorInputs() {
        const firstSuccessor = document.querySelector('.successor'); // Sélectionnez le premier champ existant
        const clonedSuccessor = firstSuccessor.cloneNode(true); // Clonez le premier champ

        successorsContainer.appendChild(clonedSuccessor);
        successorCount++;

        // Mettez à jour les noms des champs clonés pour qu'ils soient uniques
        const nameInput = clonedSuccessor.querySelector('input[name="name"]');
        nameInput.name = `name_${successorCount}`;

        const firstNameInput = clonedSuccessor.querySelector('input[name="first_name"]');
        firstNameInput.name = `first_name_${successorCount}`;

        const accessionDateInput = clonedSuccessor.querySelector('input[name="accession_date"]');
        accessionDateInput.name = `accession_date_${successorCount}`;

        // Ajoutez un gestionnaire d'événement pour supprimer le champ cloné
        const removeButton = clonedSuccessor.querySelector('.remove-successor');
        removeButton.addEventListener("click", function() {
            successorsContainer.removeChild(clonedSuccessor);
            successorCount--;
    ...
    cloneAndAppendSuccessorInputs();

    addSuccessorButton.addEventListener("click", cloneAndAppendSuccessorInputs);
});
